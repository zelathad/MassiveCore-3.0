...
public class ReflectionUtil
{
	...
	static
	{
		...
		{
			// Java 9+ module system removed access to Field.modifiers — handled gracefully via null check in makeAccessible()
		}
	}
	...
	public static void makeAccessible(Field field)
	{
		try
		{
			...
			// Java 9+ removed access to Field.modifiers, so FIELD_DOT_MODIFIERS may be null.
	...
	// Example: "v1_9_R4"
	private static String versionRaw = computeVersionRaw();
	public static String getVersionRaw() { return versionRaw; }
	private static String computeVersionRaw()
	{
		String pkg = Bukkit.getServer().getClass().getPackage().getName();
		if (pkg.length() > 23) return pkg.substring(23);
		// Paper 1.20.5+ removed the version suffix from the craftbukkit package name.
		try
		{
			// getMinecraftVersion() exists in standard Bukkit 1.13+ but not all forks
			java.lang.reflect.Method m = Bukkit.getServer().getClass().getMethod("getMinecraftVersion");
			String mcVer = (String) m.invoke(Bukkit.getServer());
			String[] parts = mcVer.split("\\.");
			return "v" + parts[0] + "_" + parts[1] + "_R1";
		}
		catch (Throwable ignored) {}
		// Ultimate fallback: modern Paper without versioned NMS packages
		return "v1_21_R1";
	}
